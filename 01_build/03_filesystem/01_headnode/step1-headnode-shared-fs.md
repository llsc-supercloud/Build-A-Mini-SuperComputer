...
```bash
    # Example to get UUID of the USB drive
    root@headnode$> blkid
...
Here we will make the drive accessible to the compute over the network. The headnode will serve as the network filesystem (NFS) server.
  -  Install `nfs-kernel-server` package as root. 

```bash
    # Install nfs-server on the headnode
    root@headnode$> apt install nfs-kernel-server -y
```
...
After editing /etc/exports, run the `exportfs -a` command to export the filesystem.   

```bash   
   root@headnode$>  exportfs -a
```

  - Confirm the export worked.

```bash
    root@headnode$>  showmount -e

...
Check to see that you have WiFi.

- Look for the WiFi symbol in the  upper right corner. 

- Open a browser from the menu in the upper left corner
  -  click on the gridded sphere (world) to initiate a browser window,
  -  or click on the Raspberry and then select internet in the drop down menu.

## Confirming the OS customizations on the headnode
...
1. Check to see that the node is named correctly. 
At the Linux command line, type the command 
```hostname```, 
the response should be 
```headnode```	
2. Check to see that SSH is enabled. At the Linux command line execute (type) the command 
```
...
### Test the SSH connection from your laptop to your headnode

!!!note "In some classrooms this will not work due to limitations required by the local Information Services team to 
maintain a secure network.  If such is the case, you will need to continue to connect from the headnode."  

While connected to the headnode via a monitor, keyboard and mouse, get the IP address of the headnode:
-   execute the command ```if config```
-   look at wlan0 to get the IP address

From your laptop, ssh to the headnode as admin using the following command, replacing wlan0-ipaddress with the address
you just obtained from the output of ifconfig
```
